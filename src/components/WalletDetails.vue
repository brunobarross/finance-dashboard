<template>
  <div class="app-surface rounded-lg p-3 sm:p-4">
    <div class="flex items-center justify-between mb-2 sm:mb-3">
      <span class="text-xs sm:text-sm font-medium app-text-muted">{{
        $t('finance.walletTransactions')
      }}</span>
      <span v-if="!wallet?.isLoading" class="text-xs app-text-muted"
    ...
    <div v-else-if="!wallet?.transactions.length" class="text-center py-3 sm:py-4">
      <p class="app-text-muted text-xs sm:text-sm">{{ $t('finance.noTransactionsMonth') }}</p>
    </div>
